...
3. `eda.py`: Contains functions for simple exploratroy data analysis, including summarizing dataset structure and content, calculating descriptive statistics, and creating visualizations (histograms, correlation matrices, boxplots, and scatter plots) to analyze distributions. 
...
2. `missing labs.ipnyb`: Focuses on visualizing missingness to confirm that the correct lab columns are being selected and imputed appropriately. Also contains a function that only selects lab columns that have less than 50% missing data (ultimately was not used in main.ipnyb). 
